...
# operating system:  Linux 4.9.0-3-amd64 x86_64 [ELF] (4.9.0)
# libc:	(6.24.0)
# binutils:	(228)
...
.SUFFIXES: .cc
    ...
    CXXDEBUGFLAGS = -g3 -fno-strict-aliasing
CXXDEPENDINCLUDES =
 CXXEXTRA_DEFINES =
CXXEXTRA_INCLUDES =
   CXXSTD_DEFINES = -Dlinux -D_POSIX_C_SOURCE=199309L 				-D_POSIX_SOURCE -D_XOPEN_SOURCE 				-D_BSD_SOURCE -D_SVID_SOURCE                                 -D_LARGEFILE_SOURCE -D_FILE_OFFSET_BITS=64 				  				 $(CXXPROJECT_DEFINES)
       CXXOPTIONS =
...
MODLDCOMBINEFLAGS = -r
  ...
  STD_CPP_DEFINES = -traditional -Dlinux -D_POSIX_C_SOURCE=199309L 				-D_POSIX_SOURCE -D_XOPEN_SOURCE 				-D_BSD_SOURCE -D_SVID_SOURCE                                 -D_LARGEFILE_SOURCE -D_FILE_OFFSET_BITS=64 				  				 $(PROJECT_DEFINES)
      STD_DEFINES = -Dlinux -D_POSIX_C_SOURCE=199309L 				-D_POSIX_SOURCE -D_XOPEN_SOURCE 				-D_BSD_SOURCE -D_SVID_SOURCE                                 -D_LARGEFILE_SOURCE -D_FILE_OFFSET_BITS=64 				  				 $(PROJECT_DEFINES)
 EXTRA_LOAD_FLAGS =
  EXTRA_LDOPTIONS =
  EXTRA_LIBRARIES =
             TAGS = ctags
   ...
   PARALLELMFLAGS =
    ...
    INSTKMEMFLAGS = -m 4711
      ...
      CDEBUGFLAGS = -g3 -fno-strict-aliasing
        CCOPTIONS =
...
# start of Imakefile
  ...
  EXTRA_LIBRARIES = -lMrm -lXm -lXt -lX11 $(DEPLIBS) $(SYSLIBS)
...
install:: $(PGM)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  $(PGM) $(DESTDIR)$(BINDIR)/$(PGM)

all:: $(PGM).$(MANNEWSUFFIX)

$(PGM).$(MANNEWSUFFIX): $(PGM).$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname $(PGM)` && \
	   $(LN) `basename $(PGM).$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < $(PGM).$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) $(PGM).$(MANNEWSUFFIX)

install.man:: $(PGM).$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) $(PGM).$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/$(PGM).$(MANSUFFIX)
...
install:: $(CLASS).ad
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) $(CLASS).ad $(DESTDIR)$(XAPPLOADDIR)/$(CLASS)
